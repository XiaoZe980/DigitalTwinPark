...
### 开发者背景
- 学历背景不占优势（双非），**正在向程序开发方向转型**
- 选择方向：**UE5 数字孪生 / 实时三维可视化**
- 当前项目是这个转型过程中的**核心实战作品**

### 为什么选数字孪生
- 智慧园区/智慧城市是政策和产业的明确需求方向
- UE5 的 Nanite / Lumen / Pixel Streaming 在三维可视化大屏场景有天然优势
- 技术栈综合度高（C++ 架构 + 蓝图 + 数据链路 + 网络部署），能一次性练到多个方向

### 项目定位
**这不只是一个 Demo，而是一份求职作品集**，目标是在面试中体现以下能力：

| 能力维度 | 项目中的体现 |
|---|---|
| **C++ 架构设计** | 策略模式数据层（`IDTPDataProvider` + Mock/HTTP 双实现）、GameInstanceSubsystem、多播委托事件驱动 |
| **引擎机制理解** | 反射调用蓝图函数、Editor 工具（`CallInEditor`）、组件生命周期、坐标/旋转数学 |
| **图形与性能** | 相机插值（四元数 Slerp）、Virtual Shadow Map 溢出排查、动态光源批量管理 |
| **数据可视化** | 实时数据链路、热力着色、告警分级、浮动标签、总览仪表盘 |
| **网络与部署** | Pixel Streaming 信令服务器搭建、浏览器端流送与输入透传 |
| **工程实践** | 结构化提交规范、大资产版本控制策略、跨机器环境交接 |

### 后续开发的取舍原则
优先做**能讲出技术含量**的功能（性能优化、架构重构、部署方案），
而不是纯堆美术效果——面试聊的是「你怎么解决的」，不是「你摆了多少模型」。
